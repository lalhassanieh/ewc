		...
		$($categories).on('click', 'a', function(e){
			...
			$.ajax({
				...
				success: function(data) {
					...
					$('.wpbingo-breadcrumb').replaceWith($(data).find('.wpbingo-breadcrumb'));
					if ( $collection ) {
						...
						if (variant.featured_media.id == imageID) {
							...
							setTimeout(function(){
								if (slickIndex !== undefined && slickIndex !== null) {
									var $quickViewImages = $(selectors.quickViewImages, selectors.quickViewContainer);
									if ($quickViewImages.hasClass('slick-initialized')) {
										$quickViewImages.slick('slickGoTo', slickIndex);
									}
								}
							}, 100);
						}
...
wpbingo.slickCarousel = function() {
	...
	$('.product-tabs__nav-link').on('shown.bs.tab', function() {
		...
		if (productTabs.find(bwpCarousel).length > 0) {
			productTabs.find(bwpCarousel).each(function() {
...
wpbingo.elementslickCarousel = function($element) {
							...
							if (variant.available) {
								if(variant.title == variants_value){
									...
									if(variant.compare_at_price){
										var $percent = Math.round(((variant.compare_at_price - variant.price)*100)/variant.compare_at_price);
										$(".product-card__on-sale",$current).css('display','block');
									...
									if(variant.featured_media){
										...
										if($('.product-card__image-wrapper',$current).hasClass('slider')){
											...
											var $productImage = $('.product-card__image',$current);
											if ($productImage.hasClass('slick-initialized')) {
												$productImage.slick('slickGoTo',variant_slick);
											}
										}else{
...
wpbingo.lookbook_block = function(){
	$(window).on("click.Bst", function(event){
		var $box3 = $('.wpbingo-widget-lookbook .content-product-card .product-card');
		var $box4 = $('.wpbingo-widget-lookbook .lookbook-card__point');
		if ( $box3.has(event.target).length == 0 && !$box3.is(event.target) && $box4.has(event.target).length == 0 && !$box4.is(event.target) ) {
			if($('.wpbingo-widget-lookbook .lookbook-card__item').hasClass("active")){
				$('.wpbingo-widget-lookbook .lookbook-card__item').removeClass("active");
			}
		}
	});
	$(".wpbingo-widget-lookbook .close-lookbook").on( "click", function() {
		if($('.wpbingo-widget-lookbook .lookbook-card__item').hasClass("active")){
			$('.wpbingo-widget-lookbook .lookbook-card__item').removeClass("active");
		}
	});
	$(".wpbingo-widget-lookbook .lookbook-card__btn").on( "click", function() {
		var $parent = $(this).closest('.wpbingo-widget-lookbook');
		var $item = $(this).closest(".lookbook-card__item");
		$('.lookbook-card__item',$parent).removeClass("active");
		if($(this).hasClass("active")){
		...
		$.each(cart.items, function(index, cartItem) {
			...
			if (cart.cart_level_discount_applications.length !== 0) {
				for (var cartDiscount in cart.cart_level_discount_applications) {
					...
					cart.cart_level_discount_applications[cartDiscount].formattedAmount = wpbingo.Currency.formatMoney(
						cartAmount,
						settings.moneyFormat
					);
				}
		  ...
		  // Create item's data object and add to 'items' array
			item = {
				...
				discountprice: wpbingo.Currency.formatMoney(
					cartItem.original_line_price,
					settings.moneyFormat
				),
				finalPrice: wpbingo.Currency.formatMoney(
					cartItem.final_line_price,
					settings.moneyFormat
				),
				handle:cartItem.handle,
...
wpbingo.wpb_accordion = function(){
	if ($(window).width() < 768) {
		$('.list-content.active_accordion .wpbingo-title__heading').off('click').on('click', function () {
